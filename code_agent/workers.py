...
你**不能修改文件、也不能执行命令**。需要改代码或跑测试时，不要尝试调用那类工具，
直接在汇报里说明应由 coder / verifier 负责。"""
...
你**不能执行命令**（没有 run_command）。需要跑测试或命令时，不要尝试调用那类工具，
直接在汇报里说明应由 verifier 执行 —— 验证由 Verifier 负责。"""
...
你**不能修改文件**。发现代码有问题时，只报告问题与建议，不要尝试调用改代码的工具。"""
